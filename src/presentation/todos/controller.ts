...
export class TodosController {
  ...
  public updateTodo = async (req: Request, res: Response) => {
    ...
    const todoFound = await prisma.todo.findFirst({ where: { id } });
    if (!todoFound) return res.status(404).json({ error: `Error: todo with id ${id} not found` });
  ...
  public deleteTodo = async (req: Request, res: Response) => {
    ...
    return todoDeleted ? res.json(todoDeleted) : res.status(400).json({ error: `Error: todo with id ${id} not found` });
  };
}
